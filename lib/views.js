...
class View extends react_1.Component {
     ...
     */
    subscribe(store, eventName, callback, receiveLastValue = false) {
        const subscription = store.subscribe(eventName, callback, receiveLastValue);
        // on an event where the component is unmounted and then mounted:
        __classPrivateFieldGet(this, _toBeSubscribedIfUnMounted).push({ store, eventName, callback, receiveLastValue });
     ...
     */
    subscribeTo(event, callback, receiveLastValue = false) {
        const subscription = event.subscribe(callback, receiveLastValue);
        __classPrivateFieldGet(this, _subscriptions).push(subscription);
        // on an event where the component is unmounted and then mounted:
        __classPrivateFieldGet(this, _toBeSubscribedIfUnMounted).push({ event, callback, receiveLastValue });
        return subscription;
    }
    componentDidMount() {
        if (!__classPrivateFieldGet(this, _hasBeenUnmounted))
            return;
        const that = this;
        __classPrivateFieldGet(this, _toBeSubscribedIfUnMounted).forEach(observer => {
...
exports.View = View;
_subscriptions = new WeakMap(), _toBeSubscribedIfUnMounted = new WeakMap(), _hasBeenUnmounted = new WeakMap();
